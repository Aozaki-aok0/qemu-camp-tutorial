...
理解了这些概念之后，我们用一个具体的例子来看看 QEMU 是如何组织 virt 主板的地址空间的。通过 QEMU 的 monitor 控制台，可以直接查看当前的地址映射关系。
